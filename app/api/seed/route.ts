...
import { OFFERTE_SEED, PARAMETRI_SEED, ARGOMENTI_SEED, FASCE_RETE_SEED } from '@/lib/seedData';
 ...
 *
 * NOTA: risemina anche le fasce di rete (Admin → "Rete e oneri"). Se le hai
 * già modificate a mano, rilanciare il seed le riporta ai valori di partenza.
 */
export async function GET(req: NextRequest) {
  ...
  return NextResponse.json({
    ok: true,
    messaggio: `Caricate ${offerte} offerte, ${parametri} parametri di dettaglio, ${argomenti} argomenti di vendita e ${fasce} fasce di rete.`
  });
}
